...
def create_knowledge_tool(
    ...
    Returns:
        A configured TAFTool that searches the specified knowledge with injected dependencies
    Example:
        >>> tool = create_knowledge_tool(
        ...     memory_client=taf.memory_client,
        ...     knowledge_base=knowledge_base
        ...     tool_config=KnowledgeToolConfig(top_k=3),
        ... )
        >>> # LLM only sees: search_knowledge(query: str)
        >>> result = await tool(query="What is TAF?")
    """
